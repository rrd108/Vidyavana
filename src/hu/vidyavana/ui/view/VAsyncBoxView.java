...
/**
 * A copy of AsyncBoxView to fix its bugs.
 */
public class VAsyncBoxView extends View
{
	public VAsyncBoxView(Element elem, int axis)
	{
		super(elem);
		stats = new ArrayList<VChildState>();
		this.axis = axis;
	 ...
	 */
	protected VChildState getChildState(int index)
	{
		synchronized(stats)
		{
	...
	/**
	 * New VChildState records are created through
	 * this method to allow subclasses the extend
	 * the VChildState records to do/hold more
	 */
	protected VChildState createVChildState(View v)
	{
		return new VChildState(v);
	}
	 ...
	 * the given VChildState object when it has completed
	 ...
	 */
	protected synchronized void majorRequirementChange(VChildState cs, float delta)
	{
	 ...
	 * model/view translation).
	 */
	protected synchronized void minorRequirementChange(VChildState cs)
	{
		minorChanged = true;
	}
			...
			synchronized(this)
			{
				...
				synchronized(stats)
				{
					...
					if((n > 0) && (minorChanged || estimatedMajorSpan))
					{
						...
						VChildState min = getChildState(0);
						VChildState pref = getChildState(0);
						float span = 0f;
						for(int i = 1; i < n; i++)
						{
							VChildState cs = getChildState(i);
							if(minorChanged)
							{
								if(cs.min > min.min)
								{
	...
	public void replace(int offset, int length, View[] views)
	{
		synchronized(stats)
		{
			...
			{
				VChildState cs = stats.remove(offset);
			...
			if(views != null)
			{
				...
				{
					VChildState s = createVChildState(views[i]);
								...
								DocumentEvent e, Shape a)
	{
		if(ec != null)
		{
			...
			VChildState cs = getChildState(index);
	...
	void setSpanOnAxis(int axis, float span)
	{
		...
		if(axis == getMinorAxis())
		{
			...
			if(targetSpan != minorSpan)
			{
				...
				// mark all of the VChildState instances as needing to
	...
	public void paint(Graphics g, Shape alloc)
	{
		...
		{
			System.out.println("paint "+alloc);
	...
	{
		VChildState cs = getChildState(n);
		if(cs != null)
		{
			System.out.println("getView "+cs.child.getStartOffset()+" "+cs.child.getEndOffset());
	 ...
	 * on the child view with a lock on the VChildState object
	...
	{
		return getNextVisualPositionFrom(
	...
	VChildState minRequest;
	VChildState prefRequest;
	...
	public class ChildLocator
	{
		 ...
		 * This is called by a VChildState object that has
		 * changed it's major span.  This can therefore be
		 * called by multiple threads.
		 */
		public synchronized void childChanged(VChildState cs)
		{
		...
		public synchronized void paintChildren(Graphics g)
		{
			...
			for(int i = index; i < n; i++)
			{
				...
				if(intersectsClip(ca, clip))
				{
					...
					{
						View v = cs.getChildView();
						System.out.println("Painting " + v.getStartOffset() + " " + v.getEndOffset());
						v.paint(g, ca);
					}
				}
				else
		...
		public synchronized Shape getChildAllocation(int index, Shape a)
		{
			...
			System.out.println("getChildAllocation "+a);
			setAllocation(a);
			VChildState cs = getChildState(index);
			if(lastValidOffset == null)
			{
		...
		{
			System.out.println("getViewIndexAtPoint "+a);
			setAllocation(a);
		...
		/**
		 * Fetch the allocation to use for a child view.
		 * <em>This does not update the offsets in the VChildState
		 * records.</em>
		 */
		 ...
		 * on the VChildState objects up to the given target span
		 ...
		 * that is valid.
		 */
		protected VChildState lastValidOffset;
	 ...
	 * synchronization on the VChildState instance.
	 * @since 1.3
	 */
	public class VChildState implements Runnable
	{
		 ...
		 * @since 1.4
		 */
		public VChildState(View v)
		{
			child = v;
			minorValid = false;
			majorValid = false;
			childSizeValid = false;
			child.setParent(VAsyncBoxView.this);
		}
		...
		public void run()
		{
			...
			try
			{
				...
				if(child.getParent() == VAsyncBoxView.this)
				{
					...
					// overwrite.
					synchronized(VAsyncBoxView.this)
					{
						changing = this;
					}
					updateChild();
		...
		// minor axis
		protected float min;
		protected float pref;
		protected float max;
		protected boolean minorValid;

		// major axis
		protected float span;
		protected float offset;
		protected boolean majorValid;

		protected View child;
		protected boolean childSizeValid;
	}
	...
	/**
	 * Copied from Utilies due to its package private nature.
	 */
